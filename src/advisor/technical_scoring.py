...
def analyze_options_data(ticker: str, current_price: float) -> dict:
    ...
    try:
        from src.option_analyst import calculate_gex, calculate_pcr, calculate_atm_iv, calculate_max_pain
        ...
        if iv:
            res["atm_iv"] = iv
            if iv > 0.4: res["score_adj"] -= 0.2
            
        # Max Pain
        mp = calculate_max_pain(ticker)
        if mp:
            res["max_pain"] = mp
            if current_price < mp * 0.95: res["score_adj"] += 0.3
            elif current_price > mp * 1.05: res["score_adj"] -= 0.3
...
def calc_trend_score(ma, macd, ichi, adx) -> float:
    ...
    # Ichimoku
    if ichi["sannyaku"]: score += 1.0
    elif ichi["regime"] == "above_cloud": score += 0.5
    elif ichi["regime"] == "below_cloud": score -= 0.7
    elif ichi["regime"] == "in_cloud": score *= 0.5
    
    # MA
    if ma == "上昇トレンド": score += 0.5
    elif ma == "下降トレンド": score -= 0.5
...
def calc_momentum_score(dyn_rsi, stoch, div) -> float:
    score = 0.0
    # Dynamic RSI
    if dyn_rsi["signal"] == "売られすぎ": score += 1.0
    elif dyn_rsi["signal"] == "買われすぎ": score -= 1.0
    ...
    # Divergence
    if div == "bullish": score += 0.8
    elif div == "bearish": score -= 0.8
...
def calc_pattern_score(bb_sq, bb, ma_dev, pv, cdl) -> float:
    score = 0.0
    # BB Squeeze
    if bb_sq["signal"] == "expansion_breakout": score += 1.0
    elif bb_sq["squeeze"]: score += 0.2
    
    # BB Position
    if bb["position"] == "下限突破": score += 0.5
    elif bb["position"] == "上限突破": score -= 0.5
    
    # Peak/Valley Structure
    if pv["signal"] == "higher_highs": score += 0.5
    elif pv["signal"] == "lower_lows": score -= 0.5
    
    # Candlestick
    score += cdl["score_adjustment"]
    
    # MA Deviation Reversion
    if ma_dev < -15: score += 0.7
    elif ma_dev < -5: score += 0.3
    elif ma_dev > 15: score -= 0.7
    elif ma_dev > 5: score -= 0.3
...
def calc_flow_score(obv, option_adj) -> float:
    score = 0.0
    # OBV
    if obv["divergence"] == "bullish": score += 1.0
    elif obv["divergence"] == "bearish": score -= 1.0
    if obv["trend"] == "上昇": score += 0.3
    elif obv["trend"] == "下降": score -= 0.3
