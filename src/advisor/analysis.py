...
from src.market_data import get_stock_data, get_stock_info, get_market_indices, get_stock_news
...
def get_theme_exposure_analysis(holdings: list[dict]) -> dict:
    ...
    if total_value > 0:
        return {
            ...
            for theme, val in sorted(theme_values.items(), key=lambda x: x[1], reverse=True)[:10]
        }
    ...
    for line in lines[1:]:  # ヘッダーをスキップ
        ...
        if len(parts) >= 2:
            ...
            try:
                ...
                avg_cost = float(parts[2].strip()) if len(parts) > 2 and parts[2].strip() else None
