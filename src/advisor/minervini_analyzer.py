...
def detect_vcp(data: pd.DataFrame, min_contractions: int = 2, max_contractions: int = 4, vol_reduction_threshold: float = 0.5) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
    ...
    try:
        ...
        pivot_highs = df['High'][(df['High'].shift(1) < df['High']) & (df['High'].shift(-1) < df['High'])]
