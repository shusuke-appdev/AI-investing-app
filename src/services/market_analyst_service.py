    ...
    try:
        ...
        if option_analysis and len(option_analysis) > 0:
            ...
            if isinstance(pcr_dict, dict):
                pcr_value = pcr_dict.get("volume_pcr")
                if isinstance(pcr_value, (int, float)):
                    opt_pcr = float(pcr_value)
        ...
        index_pe = {
            "SPY": float(spy_pe) if spy_pe is not None else None,
            "NDX": float(ndx_pe) if ndx_pe is not None else None,
        }

        spread = evaluate_yield_spread(tnx_yield, index_pe)
        yield_label = f"{tnx_yield:.2f}%" if tnx_yield is not None else "利用不可"
        advanced_tech_parts.append(
            f"\n【イールドスプレッド (10年債利回り: {yield_label})】"
        )
        for idx, res in spread["spreads"].items():
            advanced_tech_parts.append(
                f"- {idx}: 益回り {res['earnings_yield']:.2f}% (スプレッド: {res['spread']:.2f}%) -> {res['status']}"
            )
        for warning in spread.get("warnings", []):
            advanced_tech_parts.append(f"- データ制約: {warning}")
