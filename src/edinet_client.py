...
def get_company_finance(ticker: str, limit: int = 4) -> dict | None:
    ...
    if len(code) != 4:
         logger.debug(f"Ticker {ticker} does not appear to be a standard 4-digit Japanese stock code.")
         return None
            ...
            if "有価証券報告書" in title or "四半期報告書" in title:
                try:
                    ...
                    if net_sales is not None:
                        record = {
                            "date": doc.period_end if hasattr(doc, 'period_end') else (doc.filing_datetime.strftime("%Y-%m-%d") if hasattr(doc.filing_datetime, "strftime") else str(doc.filing_datetime)),
                            "type": title,
                            "net_sales": net_sales,
