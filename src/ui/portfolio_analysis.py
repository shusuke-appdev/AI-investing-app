...
from src.portfolio_advisor import (
    PortfolioHolding, analyze_portfolio, generate_portfolio_advice
)
...
def render_theme_exposure(holdings: list[dict]):
    ...
    sorted_themes = sorted(theme_exposure.items(), key=lambda x: x[1], reverse=True)[:10]
...
def render_holding_card(holding: dict):
    ...
    if tech:
        icon = "🟢" if tech.overall_signal == "強気" else "🔴" if tech.overall_signal == "弱気" else "⚪"
    else:
        icon = "⚪"
    
    with st.expander(f"{icon} **{holding['ticker']}** - {holding['name']} ({holding['weight']:.1f}%)", expanded=False):
        cols = st.columns(4)
