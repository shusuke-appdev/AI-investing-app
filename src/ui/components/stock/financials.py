    ...
    try:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    size=8, color="#FFFFFF", line=dict(width=2, color="#FB8C00")
                ),
                yaxis="y2",
            )
        )
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                title="金額 (百万ドル)", side="left", showgrid=True, gridcolor="#F1F3F4"
            ),
            yaxis2=dict(
                title="利益率 (%)",
                side="right",
                ...
                showgrid=False,
                range=[
                    min(net_margin) * 1.2 if min(net_margin) < 0 else 0,
                    max(net_margin) * 1.2,
                ],
            ),
            ...
            xaxis=dict(tickmode="array", tickvals=dates),
        )
