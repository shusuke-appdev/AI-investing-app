...
def _render_detail_section(tech) -> None:
    ...
    with h1:
        ichi_icon = "☁️" if tech.ichimoku_regime == "in_cloud" else "☀️" if tech.ichimoku_regime == "above_cloud" else "🌧️"
        st.caption(f"一目: {ichi_icon} {tech.ichimoku_signal}")
        if tech.ichimoku_sannyaku:
            st.caption("✨ 三役好転")
    with h2:
        slope_map = {"bottoming": "⬆️底打ち", "topping": "⬇️天井", "rising": "↗上昇", "falling": "↘下降", "neutral": "→横"}
        st.caption(f"MACD Hist: {slope_map.get(tech.macd_hist_slope, tech.macd_hist_slope)}")
        st.caption(f"ゼロライン: {'上' if tech.macd_zero_filter == 'above_zero' else '下'}")
    with h3:
    ...
    if tech.avwap_ytd > 0:
        ...
        v1, v2 = st.columns(2)
        with v1:
            st.caption(f"AVWAP(YTD): ${tech.avwap_ytd:.2f} (乖離: {tech.avwap_deviation:+.1f}%)")
        with v2:
            if tech.gex_regime:
    ...
    p1, p2 = st.columns(2)
    with p1:
        pv_map = {"higher_highs": "📈 HH/HL (上昇構造)", "lower_lows": "📉 LH/LL (下降構造)", "range": "↔️ レンジ", "unknown": "—"}
        st.caption(f"極値構造: {pv_map.get(tech.peak_valley_signal, tech.peak_valley_signal)}")
    with p2:
        if tech.candlestick_patterns:
            cdl_label_map = {
                "engulfing": "包み足", "hammer": "ハンマー", "invertedhammer": "逆ハンマー",
                "morningstar": "明けの明星", "eveningstar": "宵の明星",
                "3whitesoldiers": "赤三兵", "3blackcrows": "黒三兵",
                "doji": "同事線", "shootingstar": "流れ星", "hangingman": "首吊り線",
            }
            names = [f"{'🟢' if p['signal'] > 0 else '🔴'} {cdl_label_map.get(p['name'], p['name'])}" for p in tech.candlestick_patterns]
