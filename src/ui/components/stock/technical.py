...
def _render_detail_section(tech) -> None:
    ...
    if getattr(tech, "skew", None) is not None or getattr(
        tech, "mr_parabolic_state", {}
    ):
        st.divider()
        ...
        with o1:
            ...
            if mr_p.get("is_parabolic"):
                ...
                dev_str = (
                    f"MA10乖離 {dev10:+.1%}" if dev10 is not None else "MA大きく乖離"
                )
        ...
        with o2:
            ...
            if skew_val is not None:
                skew_icon = (
                    "🔴" if skew_val > 0.05 else "🟢" if skew_val < -0.05 else "⚪"
                )
