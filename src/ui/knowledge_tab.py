...
from src.knowledge_storage import (
    load_all_knowledge, delete_knowledge,
    KnowledgeItem, save_knowledge,
    get_knowledge_by_id, update_knowledge
)
...
def _render_knowledge_list():
    ...
    with col1:
        st.subheader("保存済み知識")
    with col2:
        if st.button("➕ 新しい知識を追加", key="add_knowledge_btn_main", type="primary", use_container_width=True):
    ...
    if not items:
        st.info("まだ知識が追加されていません。「追加」ボタンから情報を登録してください。")
        return
    ...
    for item in items:
        ...
        with st.container(border=True):
            ...
            with cols[2]:
                ...
                if st.button("🗑️ 削除", key=f"del_{item.id}", type="primary", use_container_width=True):
...
def _render_knowledge_add():
    ...
    with col1:
        if st.button("💾 保存してAIに学習させる", type="primary", use_container_width=True, disabled=not content):
            ...
            save_knowledge(item)
            st.toast("✅ 知識を保存しました！AIチャットで利用可能になります。", icon="🎉")
...
def _render_knowledge_edit():
    ...
    new_summary = st.text_area("要約 (AIが参照する内容)", value=item.summary, height=200)
