...
from src.portfolio_history import (
    save_snapshot, get_value_series, calculate_returns,
    list_portfolios_with_history, compare_portfolios
)
...
def render_history_view():
    ...
    if not portfolios:
        st.info("履歴データがありません。ポートフォリオを分析して保存すると履歴が記録されます。")
        return
    ...
    if returns.get("period_return") is not None:
        ...
        with cols[3]:
            st.metric(f"{returns['days']}日間リターン", f"{returns['period_return']:+.2f}%")
...
def render_comparison_view():
    ...
    for pf in comparison["portfolios"]:
        summary_data.append({
            "ポートフォリオ": pf["name"],
            "現在評価額": f"${pf['current_value']:,.0f}" if pf["current_value"] else "-",
            f"{days}日間リターン": f"{pf['period_return']:+.2f}%" if pf.get("period_return") is not None else "-"
        })
    ...
    if alerts:
        for alert in alerts:
            with st.expander(f"📌 {alert['portfolio_name']} - {alert['alert_type']}"):
                ...
                st.write(f"**閾値**: {_format_threshold(alert['alert_type'], alert['threshold'])}")
                st.write(f"**有効**: {'✅' if alert['enabled'] else '❌'}")
                
                if st.button(f"🗑️ 削除", key=f"del_alert_{alert['portfolio_name']}_{alert['alert_type']}"):
