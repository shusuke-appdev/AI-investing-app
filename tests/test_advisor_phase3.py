...
from src.advisor.market_monitor import track_distribution_days, evaluate_yield_spread
...
def test_mode_selector():
    ...
    mode_info = determine_analysis_mode(close_prices, stage=2, rsi=60.0, long_term_ma_deviation=0.1)
    assert mode_info["mode"] == "trend_following"
    assert mode_info["weights"]["trend"] == 0.40
    
    # ミーンリバージョンモード（逆張り）のテスト
    # 長期MAを大きく下回る、Stage 4
    mode_info_mr = determine_analysis_mode(close_prices, stage=4, rsi=25.0, long_term_ma_deviation=-0.3)
...
def test_market_monitor_distribution_days():
    ...
    df = pd.DataFrame({
        "Close": [100, 101, 100.5, 99.5, 100],
        "Volume": [1000, 900, 1100, 1200, 1000]
    })
