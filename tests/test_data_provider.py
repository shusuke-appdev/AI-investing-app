...
class TestDataProvider:
    ...
    @patch('src.data_provider.get_quote')
    @patch('src.data_provider.is_configured', return_value=True)
    def test_get_stock_info_structure(self, mock_is_conf, mock_quote, mock_basic, mock_profile):
        """Test if get_stock_info returns correct StockInfo TypedDict structure."""
        
        # Mock responses
        mock_profile.return_value = {
            "name": "Test Inc.", "ticker": "TEST", "finnhubIndustry": "Tech",
            "marketCapitalization": 1000, "shareOutstanding": 50
        }
        mock_basic.return_value = {
            "metric": {"peTTM": 20.5, "52WeekHigh": 150}
        }
    ...
    @patch('src.data_provider.get_company_news')
    @patch('src.data_provider.is_configured', return_value=True)
    def test_get_stock_news_structure(self, mock_is_conf, mock_news):
        """Test news item structure."""
        mock_news.return_value = [{
            "headline": "Big News", "source": "WSJ", "url": "http://...", 
            "datetime": 1700000000, "summary": "Summary"
        }]
