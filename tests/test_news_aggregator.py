...
class TestMergeWithFinnhubNews:
    """merge_with_finnhub_news関数のテスト"""

    def test_deduplicates_news(self):
        """重複するニュースを排除"""
        from src.news_aggregator import merge_with_finnhub_news
        ...
        merged = merge_with_finnhub_news(gnews, finnhub, max_total=10)

        # 重複排除されて3件のはず
        assert len(merged) == 3
        # Finnhubが優先されるのでNews AはFinnhubソース（または指定なしならFinnhub付与）
    ...
    def test_respects_max_total(self):
        ...
        gnews = [
            {"title": f"GNews {i}", "link": f"https://g.com/{i}"} for i in range(10)
        ]
        finnhub = [
            {"title": f"FH {i}", "link": f"https://fh.com/{i}"} for i in range(10)
        ]

        merged = merge_with_finnhub_news(gnews, finnhub, max_total=5)
...
class TestGetAggregatedNews:
    ...
    def test_aggregates_from_multiple_sources(self, mock_get_gnews):
        ...
        news = get_aggregated_news(
            categories=["BUSINESS"], keywords=["Fed"], max_per_source=5, max_total=20
        )
