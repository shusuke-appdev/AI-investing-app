...
class TestCalculatePCR:
    ...
    def test_calculates_pcr_correctly(self):
        ...
        mock_puts = pd.DataFrame({
            'volume': [150, 250],
            'openInterest': [1500, 2500]
        })
        
        with patch('src.option_analyst.get_option_chain', return_value=(mock_calls, mock_puts)):
            from src.option_analyst import calculate_pcr
...
class TestCalculateGEX:
    ...
    def test_returns_none_when_price_unavailable(self):
        """株価が取得できない場合はNoneを返す"""
        mock_calls = pd.DataFrame({'strike': [100], 'openInterest': [1000], 'gamma': [0.05]})
        mock_puts = pd.DataFrame({'strike': [100], 'openInterest': [1000], 'gamma': [0.05]})
        
        with patch('src.option_analyst.get_option_chain', return_value=(mock_calls, mock_puts)):
            with patch('src.option_analyst.yf.Ticker') as mock_ticker:
