...
## 2026-07-01 根拠一致度ヘルスと機能間連携の修正

- `StockSignalContext` に `purchase_evidence_health` を追加し、Stock UI、
  トレード分析、AI Stock Recap入力、Data Qualityが同じ根拠一致度入力ヘルスを
  参照するようにした。
- 根拠一致度の必須4入力（テクニカル、Entry、適応型ファンダメンタル、
  テーマ順位）と、確率/FOMO/Stage/部分評価による上限判定を別行で表示する。
- オプションtarget-DTE追加後のテストダブルを更新し、MarketData/yfinanceの
  horizon引数が検証経路で欠落しないようにした。

## 2026-06-24 P0データ契約・調査表現の修正

- 確率シグナルの履歴不足時は期待リターン、上昇確率、リスク調整値、
  相場適合度を数値ゼロへ変換せず「算出不可」とする。
- 根拠一致度はテクニカル、Entry、適応型ファンダメンタル、テーマ順位を
  必須入力とし、テーマ順位欠損を0点として総合点へ混ぜない。
- 通常の米国Stock取得ではテーマETFオプションを保存済みキャッシュからのみ
  任意反映し、MarketData.appやyfinanceのライブ取得を暗黙実行しない。
- Stock AIと主要評価ラベルを、売買推奨ではなく調査スタンス、確認条件、
  無効化条件、根拠一致度として表示する。
