...
def test_options():
    ...
    try:
        ...
        print(f"yfinance expirations for {ticker}: {expirations[:5] if expirations else 'None'}")
        if expirations:
             opt = stock.option_chain(expirations[0])
             print(f"yfinance calls shape: {opt.calls.shape}")
             # check yf columns to compare with OpenBB
             print(f"yfinance call columns: {opt.calls.columns.tolist() if not opt.calls.empty else 'Empty'}")
